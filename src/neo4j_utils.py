...
def initialize_neo_db(neo_driver):
    ...
    with neo_driver.session() as session:
        for _, order in orders_df.iterrows():
            details: pd.DataFrame = order_details_df[order_details_df['id_pedido'] == order['id_pedido']]
...
def new_order(tx, provider_id, order_id, expected_delivery_date, order_details):
    # Ensure Provider and Order nodes exist and connect them
    tx.run(
        """
        MERGE (p:Provider {id: $provider_id})
        MERGE (o:Order {id: $order_id})
        ON CREATE SET o.expected_delivery_date = date($expected_delivery_date)
        MERGE (p)-[:Ordered]->(o)
        """,
